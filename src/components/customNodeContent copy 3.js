      ...
      <div style={styles.nodeDetails}>
        {props.data.team === "" ? (
          ...
          <div style={styles.nodeTeam}>
            ...
            {props._children && props._children.slice(0, 4).map((child) => (
              <img
                key={child.data.id}
            ...
            {Array.isArray(props.data.members) && props.data.members.length > 0 ? (
              props.data.members.map((member, index) => (
                <div key={index} style={{color: 'white'}}>{member.name}</div>
              ))
            ) : (
